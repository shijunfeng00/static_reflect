...
namespace refl
{
	...
	struct IntegerWrapper //对于整数的包装类，比如IntegerWrapper<1234>，用于计算类型哈希值，该部分未来会废弃
	{
	...
	struct Method
	{
		...
		{
			return std::string_view{refl::get_type_name<decltype(return_type())>().str};
		}
	...
	struct Reflection
	{
		...
		static void for_each(auto tp,auto callback)
		{
			if constexpr(index<std::tuple_size<std::remove_reference_t<std::remove_const_t<decltype(tp.metadata)>>>::value)
			{
				callback(index,std::get<index>(tp.metadata));
				for_each<index+1>(tp,callback);
			}
		}
		...
		static consteval auto regist_class(FieldsType fields_info)
		{
			return Class{TypeWrapper<Object>{},fields_info,Reflection::regist_method()};
		}
		...
		{
			if constexpr(std::tuple_size<decltype(metadata)>::value==0)
			{
